...
const investigatePopebotCodebaseTool = tool(
  async ({ investigation_prompt }) => {
    try {
      const { randomUUID } = await import('crypto');
      const containerName = `investigate-${randomUUID().slice(0, 8)}`;
      ...
      await runHeadlessCodeContainer({
        ...
        branch: 'main',
        featureBranch: 'main',
        taskPrompt: investigation_prompt,
        mode: 'investigate',
      });
      ...
      console.error('[investigate_popebot_codebase] Failed:', err);
      return JSON.stringify({
        success: false,
  ...
  {
    name: 'investigate_popebot_codebase',
    description:
      'Launch a read-only Claude Code container to investigate the thepopebot codebase. The container clones the repo, explores the code using Claude Code in read-only mode, and streams findings live into the chat. Use this when you need to understand how the system works — event handler, Docker agent, API routes, database, GitHub Actions, deployment, or file structure. Also use before modifying config files (CRONS.json, TRIGGERS.json, etc.) to understand advanced fields. Results stream directly into this conversation. NOT for skill creation (use get_skill_building_guide for that).',
    schema: z.object({
      investigation_prompt: z.string().describe(
        'What to investigate in the codebase. Be specific about what you want to understand — e.g. "How does the cron system work? Show me the schema for CRONS.json entries including all optional fields." or "What database tables exist and what are their schemas?"'
      ),
    }),
  }
);
...
const getSkillBuildingGuideTool = tool(
  ...
  },
  {
    ...
    description:
      'Load the skill building guide and a full inventory of all skills (active and available-but-inactive). You MUST call this before creating or modifying any skill — the guide contains required file structure, naming conventions, SKILL.md frontmatter format, activation steps, and testing procedures that are only documented there. Also use this when you need to check what skills already exist — it shows both active and inactive skills with descriptions. Skills are lightweight bash/Node.js wrappers in `skills/` that extend what agents can do. NOT for understanding the system architecture (use investigate_popebot_codebase for that).',
    schema: z.object({}),
  }
);
...
export { createJobTool, getJobStatusTool, investigatePopebotCodebaseTool, getSkillBuildingGuideTool, getSkillDetailsTool, createStartCodingTool, createStartHeadlessCodingTool, createGetRepositoryDetailsTool };

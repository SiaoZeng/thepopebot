  ...
  try {
    ...
    for await (const event of stream) {
      ...
      if (msgType === 'ai') {
        ...
        if (headlessToolName === 'start_headless_coding' || headlessToolName === 'investigate_popebot_codebase') {
          try {
    ...
    if (headlessContainer) {
      try {
        ...
        if (exitCode === 0) {
          const isInvestigation = headlessContainer.toolName === 'investigate_popebot_codebase';
